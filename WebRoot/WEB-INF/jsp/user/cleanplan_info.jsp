...
<html>
  <head>
    <title>cleanplan_info</title>
    <jsp:include page="/common/header.jsp"></jsp:include>
    <link rel="stylesheet" href="${path }/css/childframe.css">
  </head>
  	
  <body>
  	<form id="list_info" action="" title="${path }/user/cleanplan_info.action" method="post" enctype="multipart/form-data">
  	  <div class="row">
  	  	<div class="col-xs-2">
	  ...
	  <tbody>
	  	<c:forEach var="item" items="${pageResult.items }" varStatus="status">
		    <tr>
		      ...
		      <td>${item.content }</td>
		      <td><span title="date">
		      <input type="hidden" value="${item.begin }">
		      </span></td>
		      <td><span title="date">
		      <input type="hidden" value="${item.end }">
		      </span></td>
		      <td>
